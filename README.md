...
that sale's own profit):

- `tink_small.wav` — small TINK
- `tink_medium.wav` — medium TINK
- `tink_large.wav` — large TINK
- `warning.wav` — any loss
...
These are currently placeholder tones generated by
`scripts/generate_sounds.py` (synthesized, not real game audio — see
that script's docstring for why). Drop in real files under the same
names to replace them; no code changes needed. Playback also respects
the master volume / enable-TINK / enable-warnings toggles in the same
Settings section.
